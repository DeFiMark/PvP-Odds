...
public class CaptainSlots {
		...
		public static final Coins[][] combos = {
				...
				{Coins.BNB, Coins.DOG, Coins.XRP},
				{Coins.BNB, Coins.DOG, Coins.DOG},
				
				{Coins.BNB, Coins.XRP, Coins.BTC},
				{Coins.BNB, Coins.XRP, Coins.ETH},
				{Coins.BNB, Coins.XRP, Coins.BNB},
				{Coins.BNB, Coins.XRP, Coins.SOL},
				{Coins.BNB, Coins.XRP, Coins.XRP},
				
				{Coins.BNB, Coins.SOL, Coins.BTC},
				{Coins.BNB, Coins.SOL, Coins.ETH},
				{Coins.BNB, Coins.SOL, Coins.BNB},
				{Coins.BNB, Coins.SOL, Coins.SOL},
				...
				{Coins.DOG, Coins.ETH, Coins.BNB},
				{Coins.DOG, Coins.ETH, Coins.DOG},
				
				{Coins.DOG, Coins.BNB, Coins.BTC},
				{Coins.DOG, Coins.BNB, Coins.ETH},
				{Coins.DOG, Coins.BNB, Coins.BNB},
				...
				{Coins.DOG, Coins.SOL, Coins.ETH},
		 									...
		 									//   0		 1		2	   3	  4	     5
		public static final String[] options = {"BTC", "ETH", "BNB", "SOL", "XRP", "DOGE"};
									    // BTC   ETH   BNB   SOL   XRP  DOGE
		public static final int[] reel1 = { 1,    2,    4,    6,    8,   9 };
		public static final int[] reel2 = { 1,    2,    4,    6,    8,   9 };
		public static final int[] reel3 = { 1,    2,    4,    6,    8,   9 };

		public static double payoutBasisPoint = 140;
		...
		public static void run() {
			...
			for (int i = 0; i < combos.length; i++) {
				...
				realPayouts[i] = ((double)Math.round((100 * payoutBasisPoint ) / (double)waysToMakeCombo[i])) / 100.0;
//				realPayouts[i] = ((double)Math.round((100_000.0 * payoutBasisPoint ) / ((double)waysToMakeCombo[i] / 1000))) / 100_000.0;
				
				if (realPayouts[i] < 0.5) {
					System.out.println("Payout Under 0.5: " + combos[i][0] + " " + combos[i][1] + " " + combos[i][2] + ": " + realPayouts[i]);
					realPayouts[i] = 0.25;
				} else if (realPayouts[i] < 0.7) {
				...
				else if (combos[i][0] == Coins.ETH && combos[i][1] == Coins.ETH && combos[i][2] == Coins.BTC) {
					realPayouts[i] = 40;
				}
				else if (combos[i][0] == Coins.ETH && combos[i][1] == Coins.BTC && combos[i][2] == Coins.ETH) {
					realPayouts[i] = 40;
				}
				else if (combos[i][0] == Coins.BTC && combos[i][1] == Coins.ETH && combos[i][2] == Coins.ETH) {
					realPayouts[i] = 40;
				}
			...
			int[][] sortedCombos = new int[allCombos.length][4];
			
			int[][][] comboLengths = new int[NUM_COINS][NUM_COINS][1];
			for (int startingNum = 0; startingNum < NUM_COINS; startingNum++) {
			    for (int secondNum = 0; secondNum < NUM_COINS; secondNum++) {
			        int usedInCombo = 0;
			        // Iterate over all combinations to find those matching this group.
			        for (int i = 0; i < allCombos.length; i++) {
			            if (!used[i] && allCombos[i][0] == startingNum && allCombos[i][1] == secondNum) {
			                sortedCombos[index][0] = allCombos[i][0];
			                sortedCombos[index][1] = allCombos[i][1];
			                sortedCombos[index][2] = allCombos[i][2];
			                sortedCombos[index][3] = allCombos[i][3];
			                index++;
			                usedInCombo++;
			                used[i] = true;
			            }
			        }
			        // Record the number of combos for this group.
...
//			System.out.println(allCombos[unused][0] + ", " + allCombos[unused][1] + ", " + allCombos[unused][2] + ", " + allCombos[unused][3]);
			...
			System.out.println("\n\nPrinting Combos Over 0.75% Chance To Hit\n\n");
			for (int i = 0; i < combos.length; i++) {
			...
			for (int i = 0; i < runs; i++) {
				if (i > 0 && i % SPINS_PER_GAME == 0) {
					int _index = (i / SPINS_PER_GAME) - 1;
					if (_index < GAMES_TO_TRACK) {
						gameOutputs[_index] = totalWonThisGame - gameBuyIn;
						totalWonThisGame = 0;
					}
				}
